  ...
  return (
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={onCancel}>
      ...
      >
        ...
        <p className="text-sm text-[#1A1A1A]/70 mb-6">{message}</p>
        <div className="flex gap-3 justify-end">
          <button
            onClick={onCancel}
            className="px-4 py-2 text-sm font-semibold text-[#1A1A1A]/70 hover:bg-neutral-100 rounded-xl transition-colors"
          >
            Cancel
          </button>
          <button
  ...
  return (
    <div className="fixed inset-0 bg-black/40 z-50 flex justify-end" onClick={onClose}>
      <div
        ...
        onClick={(e) => e.stopPropagation()}
      >
        <div className="sticky top-0 bg-white border-b border-[#00592B]/10 px-6 py-4 flex items-center justify-between z-10">
          <div>
            <h2 className="text-xl font-bold text-[#00592B]">{detail.name || 'Unnamed'}</h2>
            <p className="text-sm text-[#1A1A1A]/60">{detail.email}</p>
          </div>
          <button
            onClick={onClose}
            className="text-[#1A1A1A]/40 hover:text-[#1A1A1A] text-2xl leading-none px-2"
          >
            &times;
          </button>
        </div>
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="flex flex-wrap gap-4 text-sm">
            <div className="bg-[#FAFAF8] rounded-xl px-4 py-2">
              <span className="text-[#1A1A1A]/50">Joined </span>
              <span className="font-semibold text-[#1A1A1A]">
                {detail.created_at ? new Date(detail.created_at).toLocaleDateString() : 'N/A'}
              </span>
            </div>
            <div className="bg-[#FAFAF8] rounded-xl px-4 py-2">
              <span className="text-[#1A1A1A]/50">Last Active </span>
              <span className="font-semibold text-[#1A1A1A]">
            ...
            {detail.invite && (
              <div className="bg-[#FAFAF8] rounded-xl px-4 py-2">
                <span className="text-[#1A1A1A]/50">Invite </span>
                <span className="font-mono text-xs font-semibold text-[#1A1A1A]">
                  {detail.invite.token_preview}
                </span>
              </div>
            )}
            <div className={`rounded-xl px-4 py-2 ${detail.is_active ? 'bg-[#1CE585]/10' : 'bg-red-50'}`}>
              <span className={`font-bold text-sm ${detail.is_active ? 'text-[#00592B]' : 'text-red-600'}`}>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              ...
              <div key={stat.label} className="bg-[#FAFAF8] rounded-xl p-3 text-center">
                <div className="text-xs font-semibold text-[#1A1A1A]/50">{stat.label}</div>
                <div className="text-xl font-black text-[#00592B]">{stat.value}</div>
              </div>
            ))}
          </div>
          ...
          {Object.keys(analytics.by_difficulty).length > 0 && (
            <div>
              <h3 className="text-sm font-bold text-[#1A1A1A]/70 mb-2">By Difficulty</h3>
              <div className="flex gap-3">
                {Object.entries(analytics.by_difficulty).map(([diff, count]) => (
                  <div key={diff} className="bg-[#FAFAF8] rounded-xl px-4 py-2 text-center flex-1">
                    <div className="text-xs text-[#1A1A1A]/50 capitalize">{diff}</div>
          ...
          {analytics.by_domain.length > 0 && (
            <div>
              <h3 className="text-sm font-bold text-[#1A1A1A]/70 mb-2">By Domain</h3>
              <div className="space-y-2">
                {analytics.by_domain.map((d) => (
                  <div key={d.domain} className="flex items-center gap-3">
                    <div className="flex-1">
                      <div className="flex justify-between text-sm mb-1">
                        <span className="font-medium text-[#1A1A1A]">{d.domain}</span>
                        <span className="text-[#1A1A1A]/50">
                          {d.correct}/{d.total} ({d.accuracy}%)
                        </span>
                      </div>
                      <div className="h-2 bg-[#00592B]/10 rounded-full overflow-hidden">
                        <div
                          className="h-full bg-[#1CE585] rounded-full transition-all"
            ...
            <div>
              ...
              <div className="space-y-1">
                {analytics.weak_skills.map((s) => (
                  ...
                  >
                    <span className="font-medium text-[#1A1A1A]">{s.skill}</span>
          ...
          {analytics.recent_activity.length > 0 && (
            <div>
              <h3 className="text-sm font-bold text-[#1A1A1A]/70 mb-2">Recent Activity</h3>
              <div className="space-y-1">
                {analytics.recent_activity.map((a, i) => (
                  <div
                    key={i}
                    className="flex items-start gap-3 text-sm py-2 border-b border-[#00592B]/5 last:border-0"
                  >
                    <span
                      className={`mt-0.5 w-5 h-5 rounded-full flex items-center justify-center text-xs font-bold text-white shrink-0 ${
                        a.is_correct ? 'bg-[#1CE585]' : 'bg-red-400'
                      }`}
                    >
                      {a.is_correct ? '✓' : '✗'}
                    </span>
                    <div className="flex-1 min-w-0">
                      <p className="text-[#1A1A1A] truncate">{a.snippet}</p>
            ...
            <table className="w-full">
              ...
              <tbody>
                {students.map((s) => (
                  <tr
                    key={s.id}
                    className="border-b border-[#00592B]/5 hover:bg-[#00592B]/[0.02] cursor-pointer"
                    onClick={() => openStudentDetail(s.id)}
                  >
                    <td className="px-6 py-4 font-medium text-[#1A1A1A]">
                      <div className="flex items-center gap-2">
                        {loadingStudentId === s.id && (
                          <span className="w-4 h-4 border-2 border-[#00592B]/20 border-t-[#00592B] rounded-full animate-spin" />
                        )}
                        {s.name || 'N/A'}
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-4 text-xs font-mono text-[#1A1A1A]/40">
